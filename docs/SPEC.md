...
### エフェクトレーン(8種)
1. **Stutter** — キャプチャバッファのスライスをレート(1/16, 1/32, 1/64, 1/128, 1/256、三連/付点)でループ再生。パラメータ: レート、ループ長減衰(だんだん短く=Stutter Edit 的グラデーション)、ピッチスライド量
   - ※ v1.1.2 まではこのレート表示が実際の4倍ずれていた(「1/4」を選ぶと16分音符のループ)。DSP は変えずラベルのみ修正済みで、プリセットの音は不変。`stutter::legacyRateIndexLabels()` が唯一の定義
